...
class TraceViewerComponent extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleClick() {
    if (this.props.file != null) {
      this.setState((currentState) => {
        return { file: this.props.file, showGraph: false, xmlDataStatut: "" };
      });
    ...
    if (this.state.file == null) {
      ...
      this.setState((currentState) => {
        return {
          filetitle: this.props.file.name,
        };
      });
  ...
  readfileCreateData(reader, color, compareMode) {
    ...
    if (this.state.rbConfig === "session") {
      ...
      for (i = 0; i < cellsArray.length; i++) {
        ...
        if (
          result[cellsArray[i].getAttribute("session")] === undefined ||
          !result[cellsArray[i].getAttribute("session")]
        ) {
          result[cellsArray[i].getAttribute("session")] = {
            sp: cellsArray[i].getAttribute("session"),
            min: new Date(
              "2024-03-05 " + cellsArray[i].getAttribute("timestamp")
            ).getTime(),
            max: new Date(
              "2024-03-05 " + cellsArray[i].getAttribute("timestamp")
            ).getTime(),
          ...
          if (
            new Date(
              "2024-03-05 " + cellsArray[i].getAttribute("timestamp")
            ).getTime() < result[cellsArray[i].getAttribute("session")].min
          ) {
            if (i + 1 === cells.length) {
              result[cellsArray[i].getAttribute("session")].min =
                new Date(
                  "2024-03-05 " + cellsArray[i].getAttribute("timestamp")
                ).getTime() + 10; // +10 ms par défaut
            } else {
              result[cellsArray[i].getAttribute("session")].min = new Date(
                "2024-03-05 " + cellsArray[i].getAttribute("timestamp")
              ).getTime();
            }
          }
          ...
          if (
            new Date(
              "2024-03-05 " + cellsArray[i].getAttribute("timestamp")
            ).getTime() > result[cellsArray[i].getAttribute("session")].max
          ) {
            if (i + 1 === cells.length) {
              result[cellsArray[i].getAttribute("session")].max =
                new Date(
                  "2024-03-05 " + cellsArray[i].getAttribute("timestamp")
                ).getTime() + 10; // +10 ms par défaut
            } else {
              result[cellsArray[i].getAttribute("session")].max = new Date(
                "2024-03-05 " + cellsArray[i].getAttribute("timestamp")
              ).getTime();
            }
          }
    ...
    // traitement des Cell par cell
      ...
      for (i = 0; i < cells.length; i++) {
        ...
        if (this.state.rbConfigLine === "compact") {
          dataIn.x =
            "sp-" +
            cells[i].getAttribute("sp") +
            "_" +
            cells[i].getAttribute("session");
          if (
            result[
              "sp-" +
                cells[i].getAttribute("sp") +
        ...
        if (i + 1 === cells.length) {
          dataIn.y = [
            new Date(
              "2024-03-05 " + cells[i].getAttribute("timestamp")
            ).getTime(),
            new Date(cells[i].getAttribute("timestamp")).getTime() + 10,
          ]; // +10 ms par défaut
        } else {
          dataIn.y = [
            new Date(
            ...
            new Date(
              "2024-03-05 " + cells[i + 1].getAttribute("timestamp")
            ).getTime(),
          ];
        }
        // TODO rendre les couleurs aléatoire
  ...
  handleChangeRadioButon(e) {
    ...
    else document.getElementById("radioMultiCompact").style.display = "block";
  }
    ...
    return (
      <div>
        ...
        <div className="row">
          ...
          <div className="col-sm">
            <div className="form-check">
              <input
                ...
                type="radio"
                value="cell"
                checked={this.state.rbConfig === "cell"}
                onChange={(e) => this.handleChangeRadioButon(e)}
                name="rdConfig"
                id="radioCell"
                defaultChecked
              ></input>
            ...
            <div className="form-check">
              <input
                ...
                type="radio"
                value="session"
                checked={this.state.rbConfig === "session"}
                onChange={(e) => this.handleChangeRadioButon(e)}
                name="rdConfig"
                id="radioSession"
              ></input>
          ...
          <div className="col-sm" id="radioMultiCompact">
            <div className="form-check">
              <input
                className="form-check-input"
                type="radio"
                value="multi"
                checked={this.state.rbConfigLine === "multi"}
                onChange={(e) => this.handleChangeRadioButonLine(e)}
                name="rbConfigLine"
                id="radioMulti"
                defaultChecked
              ></input>
            ...
            <div className="form-check">
              <input
                ...
                type="radio"
                value="compact"
                checked={this.state.rbConfigLine === "compact"}
                onChange={(e) => this.handleChangeRadioButonLine(e)}
                name="rbConfigLine"
                id="radioSimple"
              ></input>
            ...
            <div className="form-check">
              <input
                ...
                type="radio"
                value="compare"
                checked={this.state.rbConfigLine === "compare"}
                onChange={(e) => this.handleChangeRadioButonLine(e)}
                name="rbConfigLine"
                id="radiocompare"
              ></input>
            ...
            <input
              ...
              type="file"
              id="formFile"
              onChange={(e) => this.handleCompareFileChange(e.target.files[0])}
              accept="text/xml"
            ></input>
          </div>
          <div className="col-sm"></div>
          {
            // div pour d'autres options
          }
        </div>
        <div className="row">
          <div id="chart" className="col-12">
            {this.state.loading === true ? (
              <div className="progress">
                <div
                  className="progress-bar progress-bar-animated progress-bar-striped active"
                  role="progressbar"
                  aria-valuenow="40"
                  aria-valuemin="0"
                  aria-valuemax="100"
          ...
          <div
            className="col-4"
            style={{ top: 400, right: 0, position: "fixed" }}
          >
            <h2 id="cellName"> </h2>
            <pre lang="xml" id="cellContent" className="cellContent"></pre>
...
export default TraceViewerComponent;
